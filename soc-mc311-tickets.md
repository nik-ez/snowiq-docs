...
* **Assigned:** The Supervisor created the Assignment in the SA and sent it to the Driver, but the work has yet to begin.
...
This view provides a panel on the left that lists the tickets and their statuses along with a visual on the map of markers (color coded) that represent tickets which gives users an idea of where the tickets are located in the service area. Additionally, clusters are visible when the map is zoomed out.

Clicking on the blue arrow next to the ticket in the left panel or on the marker on the map produces a popup with the details of the ticket, as seen below. It also allows the user to either Close or Send a ticket.
...
In order for the ticket to be worked on, it must go through a sequence of steps, the first of which is to send the ticket to the Supervisor to create an assignment using the following steps:
...
* Enter the address and hit Enter, which will prompt the system to locate the address and, if located, it will display below the search bar.
...
* If the ticket was created while on the MC311 Tickets Map View, the newly created ticket is added to the list on the panel to the left with a red asterisk as  an Emergency Ticket. Its status is set to 'Sent' which means the process of sending it to the Supervisor has been completed.
</section>
</section>
